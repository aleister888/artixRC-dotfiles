...
\setmonofont{Iosevka Nerd Font Complete}
...
Si has instalado discord te darás cuenta que en Linux, no puedes compartir audio cuando compartes tu escritorio o una ventana.

\vspace{5pt}

\noindent Para solucionar este problema, cuando inicias sesión se crea un \textit{micrófono virtual} en el que puedes juntar el audio de tu micrófono real y el audio de las aplicaciones que desees.
...
Podemos automatizar este proceso, para que el usuario \textit{``usuario1"} convierta a mp3 su música, todos los dias a las 8:30, añadiendo esta linea a \path{/etc/crontab}:
...
\begin{verbatim}
0 14 * * 5 root btrfs balance start -dusage=95 -musage=95 /
\end{verbatim}

Esto realizará diversas tareas de mantenimiento en tu disco principal todos los viernes a las 14:00. Si tienes un disco diferente para tu partición home, también formateado en BTRFS, puede interesarte hacer lo mismo para tu disco /home

\begin{verbatim}
0 14 * * 5 root btrfs balance start -dusage=95 -musage=95 /home
\end{verbatim}

\subsection{Limpiar cache periódicamente}

Puedes limpiar periódicamente la caché y archivos de registro antiguos de forma periódica añadiendo estas lineas a \path{/etc/crontab} sustituyendo \textit{usuario1} por el nombre de tu usuario:
...
Nos crearía todos los miércoles a las 16:00 una copia de seguridad de la carpeta \path{~/Música} en un archivo comprimido en \path{/Resguardos/Música}
...
Sustituye \textit{usuario1} por el usuario de tu máquina, y \textit{255.255.255.255} por la dirección IP de tu máquina. Para conectarte al equipo remotamente debes tener instalado algún cliente VNC, mi recomendación es \href{https://github.com/FreeRDP/Remmina}{remmina}.
...
\begin{figure}[h!]
\fcapside[\FBwidth]{\caption*{Nuestro Firefox viene instalado con varias extensiones útiles que vienen desactivadas por defecto. Para activarlas, pulsa en el icono con forma de pieza de puzle, que está situado a la derecha de la barra de búsqueda. Te encontrarás con una lista como la de la imagen de abajo. Simplemente activa cada extensión haciendo clic en el deslizador señalado y dale a aceptar en los menús flotantes que aparezcan pidiendo confirmación.}}
{\includegraphics[width=0.5\textwidth]{images/extensions.jpg}}
\end{figure}%
...
\begin{figure}[h!]
\fcapside[\FBwidth]{\caption*{Pese a usar al configuración reforzada de Firefox, \href{https://github.com/arkenfox/user.js}{arkenfox}, esta no cambia el que el buscador por defecto de Firefox es Google. Mi recomendación es cambiar Google por DuckDuckGo.}}
...
\begin{figure}[h!]
\fcapside[\FBwidth]{\caption*{Para desactivar esta función, simplemente busca \textit{Contraseña} en el menu de configuración de Firefox y desactiva el \textit{Ask to save passwords} o \textit{Preguntar para guardar contraseñas}, en español.}}
{\includegraphics[width=0.4\textwidth,height=0.13\textwidth]{images/keepass.jpg}}
\end{figure}%
...
\begin{figure}[h!]
\fcapside[\FBwidth]{\caption*{Por defecto Firefox intentará manejar nuestras contraseñas, si ya estamos usando la extensión de Keepass que viene incluida con nuestra instalación, las ventanas de Firefox que aparecen preguntando si queremos guardar nuestra contraseña pueden ser bastante molestas.
\vspace{10pt}\\
Para desactivar esta función, simplemente busca \textit{Contraseña} en el menu de configuración de Firefox y desactiva el \textit{Ask to save passwords} o \textit{Preguntar para guardar contraseñas}, en español.}}
